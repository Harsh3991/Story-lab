...
function moreAboutKaren(parents, noOfSiblings, isNuclearFamily){
  ...
  if (typeof(parents)=="string"&& typeof(noOfSiblings)=="number"&& typeof( isNuclearFamily)){
      return true;
  }
...
function sweetTooth(totalSweets,sweetsAteByKaren,sweetsAteByKarenOnTheWay,dist){

  let remainingSweets=totalSweets-sweetsAteByKaren;
  remainingSweets=remainingSweets-(sweetsAteByKarenOnTheWay*dist);
  remainingSweets/=2
...
function convertToCelcius(tempInFarenheit){
  let temperatureCelcius=(tempInFarenheit-32)*(5/9);
...
function consoleKaren(strategiesArr){
  let allStrategies=strategiesArr.join(" ");
